...
**Figures 4 and 5** collectively illustrate the spatial distribution and volume of rebalancing flows, elucidating the operational mechanism of the proposed SAC-Soft model ($\lambda=11$).

A distinct disparity in operational behavior is observed between the two models. The spatial visualization in **Figure 4** highlights a stark contrast in rebalancing patterns. The **SAC-Hard** baseline (Left) exhibits dense, extensive flow trajectories connecting distant zones (e.g., Zone 5 to Zone 12/13), indicating an aggressive strategy that prioritizes demand fulfillment irrespective of high travel costs. In contrast, **SAC-Soft** (Right) displays significantly attenuated long-distance flows, instead concentrating on short-distance rebalancing between adjacent zones.

This observation is quantitatively corroborated by the flow volume analysis of the **Top 5 Longest Routes** in **Figure 5**.
*   **Mitigation of Inefficient Long-Haul Trips**: For the route **12 $\to$ 5 (13.2 km)**, SAC-Hard dispatched **78 vehicles**. The SAC-Soft model, recognizing the disproportionate cost relative to the benefit, substantially curtailed this flow to a mere **4 vehicles**, effectively mitigating this operational inefficiency.
*   **Retention of High-Value Long-Distance Connections**: Crucially, the model avoids indiscriminate distance minimization. For the longest route **9 $\to$ 5 (16.4 km)**, SAC-Soft maintained a high flow volume (**177 vehicles**), comparable to SAC-Hard (188 vehicles). This suggests that the **Economic Filter** is capable of discerning between "wasteful" and "necessary" long-distance trips, preserving those trajectories essential for meeting critical demand or yielding higher marginal returns.

By selectively filtering out high-cost, low-value trips (e.g., 12$\to$5) while maintaining essential connections (e.g., 9$\to$5), SAC-Soft achieves a more efficient fleet distribution. This strategic behavior explains the model's ability to maintain high service levels (Trip Margin +1.1%) while simultaneously reducing overall rebalancing costs (-5.6%), as evidenced in Table 2.
...
![Top 5 Longest Flows](/C:/Users/Administrator2/.gemini/antigravity/brain/21271faf-2a9f-484d-8886-93a9cc42e18b/uploaded_media_1770959732457.png)
...
As Autonomous Mobility-on-Demand (AMoD) services expand, efficient fleet rebalancing becomes critical to address spatiotemporal demand imbalances. However, existing hierarchical frameworks coupling Reinforcement Learning (RL) with Mixed-Integer Linear Programming (MILP) often enforce strict target distributions, compelling the system to execute high-cost, inefficient rebalancing trips even when their operational cost outweighs the potential service benefit. To address this, we propose a **Soft-Constrained Rebalancing** framework integrated with an **Economic Filter** (Slack Mechanism). By relaxing hard constraints and introducing a penalty parameter ($\lambda$) for target fulfillment, our method allows the system to weigh the cost of rebalancing against the value of meeting demand. Experimental results on a real-world New York Brooklyn dataset demonstrate that the proposed approach ($\lambda=9$) effectively mitigates the "over-rebalancing" problem. It achieved a **2.84% increase in total profit** compared to the hard-constrained baseline by reducing inefficient rebalancing costs (**5.62%**) while maintaining high service levels. Furthermore, we identify an efficiency-optimal configuration ($\lambda=11$) that improves operational efficiency by 30%, filtering out wasteful long-distance empty trips. This study confirms that a cost-aware, soft-constrained approach significantly enhances the scalability and profitability of AMoD operations.
